...
def test_sma_alpha_warmup_and_ready():
    ref = InstrumentRef("AAPL")
    tf = Timeframe(1, TimeframeUnit.MINUTE)
    alpha = SMAAlpha(SMAAlphaConfig(ref=ref, tf=tf, window=3))
    ...
    out2 = alpha.update(MarketDataAlphaInput(event=_make_bar_event(ref, tf, t1, 12.0)))
    assert alpha.ready() is False
    assert out2.is_ready is False
    assert math.isnan(out2.value)

    t2 = t1 + timedelta(minutes=1)
    out3 = alpha.update(MarketDataAlphaInput(event=_make_bar_event(ref, tf, t2, 14.0)))
    assert alpha.ready() is True
    assert out3.is_ready is True
    assert out3.value == pytest.approx(12.0)


def test_sma_alpha_rejects_mismatched_event():
    ref = InstrumentRef("AAPL")
    tf = Timeframe(1, TimeframeUnit.MINUTE)
    alpha = SMAAlpha(SMAAlphaConfig(ref=ref, tf=tf, window=2))
...
def test_sma_alpha_rejects_unsupported_event_type():
    ref = InstrumentRef("AAPL")
    tf = Timeframe(1, TimeframeUnit.MINUTE)
    alpha = SMAAlpha(SMAAlphaConfig(ref=ref, tf=tf, window=2))
