"""
Daily Breakout Futures Playground (v7.6) — GC=F via yfinance (daily bars)

Locked-in mainline defaults discovered from v7.5 roll matrix + sweeps:
- entry_breakout_mult = 2.0
- stress mode ON with tuned-ish defaults (adjust if you want)
- roll_force_flat = False
- roll_avoid_entries = True (avoid entering around roll windows)
- risk_fraction_per_trade = 0.03
- min_contracts_on_signal = 0 (debug-only, OFF by default)

Preserved critical features from v7.4/v7.5:
- MTM equity tracking (Cash + Unrealized)
- Gap-aware stop-entry fills (long: max(yh, open), short: min(yl, open))
- No-lookahead trailing stop:
  - trail candidates use HH1/LL1 and ATR1 (all shifted)
  - trail is clamped vs prior close and vs entry to prevent lookahead artifacts
  - trail updates EOD and applies starting next day
- Same-day enter-and-stop handling
- Leverage + margin constraints, slippage + commissions
- Stats printout, recent days/trades, equity plotting
- Trigger diagnostics and sizing diagnostics/histograms

NEW in v7.6:
- Eligible trigger diagnostics (entry-eligible only):
  Counts triggers only when ALL are true:
    * warmup complete
    * pos is None (flat)
    * NOT roll-blocked (if roll_avoid_entries is enabled)
  This makes trigger stats comparable across roll settings and versions.
"""
...
class Config:
    ...
    # Risk & sizing (used when fixed_contracts is None)
    ...
    # Futures contract spec (GC default)
    multiplier: float = 100.0

    # Costs
    commission_per_side: float = 2.5

    # Base slippage model (points)
    base_slippage_points: float = 0.2
    slip_k_sqrt: float = 0.002

    # Robustness switches
    entry_slip_multiplier: float = 1.0
    entry_slip_multiplier_on_breakout_day: float = 2.0  # locked default
    exit_slip_multiplier: float = 1.0

    # Stress slippage (no-lookahead: uses TR1/ATR1)
    stress_mode: bool = True
    # Tuned-ish defaults: fewer stressed days than 1.5/2.0, but still meaningful
    ...
    # Approx roll-aware trading (for GC-style active months)
    roll_active_months: Tuple[int, ...] = (2, 4, 6, 8, 10, 12)
    roll_bdays_before: int = 5
    roll_window_bdays: int = 2

    # Locked mainline default per your roll matrix conclusion:
    roll_force_flat: bool = False
    roll_avoid_entries: bool = True

    # If True: avoid entries in +/- window; else avoid from roll_date through roll_date+window
    roll_avoid_window_only: bool = True

    # Portfolio aggregation (optional)
    build_equal_weight_portfolio: bool = False
    portfolio_initial_equity: float = 100_000.0
...
@dataclass
class Diagnostics:
    # Raw triggers (may include days where you were in a position)
    ...
    # Eligible triggers (flat + ready + not roll-blocked)
    ...
    # Distributions on entry-attempt days
    desired_risk_values: List[int] = field(default_factory=list)
    capped_values: List[int] = field(default_factory=list)
...
def cap_contracts_by_constraints(
    desired: int, equity: float, price: float, cfg: Config
) -> int:
    if desired <= 0:
        return 0
    notional_per_contract = price * cfg.multiplier
...
def backtest_one(
    ...
    # Prior-day references (no lookahead)
    ...
    def unrealized_at(close_price: float) -> float:
        ...
        return points_to_dollars(pnl_points, pos.contracts, cfg.multiplier)

    def clamp_trail_on_entry(
        side: int, entry: float, c_prev: float, trail_init: float
    ) -> float:
        # Clamp to avoid any same-bar lookahead benefit
        if side == +1:
    ...
    def risk_desired_contracts(
        equity: float, entry_price: float, hard_stop: float
    ) -> int:
        risk_per_contract = points_to_dollars(
            abs(entry_price - hard_stop), 1, cfg.multiplier
        )
        return int(
            math.floor(
                (equity * cfg.risk_fraction_per_trade) / max(risk_per_contract, 1e-9)
            )
        )
    ...
    def decide_contracts(
        equity: float, entry_price: float, hard_stop: float, side_tag: str
    ) -> Tuple[int, Optional[str]]:
        """
        Returns (contracts_final, note_override).
        Records sizing distributions on entry-attempt days.
        """
        if cfg.fixed_contracts is not None:
            desired = max(int(cfg.fixed_contracts), 0)
            diag.desired_risk_values.append(desired)
            capped_pre_min = (
                cap_contracts_by_constraints(desired, equity, entry_price, cfg)
                if cfg.enforce_constraints_on_fixed
                else desired
            )
            diag.capped_values.append(capped_pre_min)
            if desired > 0 and capped_pre_min <= 0:
                diag.size0_by_constraints += 1
            return capped_pre_min, None

        desired = risk_desired_contracts(equity, entry_price, hard_stop)
        diag.desired_risk_values.append(desired)
        if desired <= 0:
        ...
        # Debug-only min floor
        if (
            capped_pre_min <= 0
            and cfg.min_contracts_on_signal
            and cfg.min_contracts_on_signal > 0
        ):
            desired_min = int(cfg.min_contracts_on_signal)
            forced = (
                cap_contracts_by_constraints(desired_min, equity, entry_price, cfg)
                if cfg.enforce_constraints_on_min
                else desired_min
            )
            if forced > 0:
                diag.forced_min_used += 1
                return forced, f"forced_min_{side_tag}"
            return 0, None
    ...
    for date, row in df.iterrows():
        ...
        ready = pd.notna([yh, yl, atr1, hh1, ll1, c1, tr1]).all()
        note = ""
        ...
        # ---- RAW trigger diagnostics (for curiosity, not comparable across settings) ----
        long_trig_raw = cfg.enable_long and (h >= yh)
        short_trig_raw = cfg.enable_short and (l <= yl)
        if long_trig_raw or short_trig_raw:
            diag.trigger_any_raw += 1
        if long_trig_raw:
            diag.trigger_long_raw += 1
        if short_trig_raw:
            diag.trigger_short_raw += 1
        if long_trig_raw and short_trig_raw:
            diag.trigger_both_raw += 1

        # 0) Roll handling (mainline default is roll_force_flat=False)
        if cfg.roll_force_flat and pos is not None and (date_d in roll_dates):
            diag.roll_force_flat_exits += 1
            diag.stressed_execs += 1 if stressed else 0

            if pos.side == +1:
                slip = slip_points(pos.contracts, stressed, cfg.exit_slip_multiplier)
                exit_price = o - slip
                gross = points_to_dollars(
                    exit_price - pos.entry_price, pos.contracts, cfg.multiplier
                )
                cash += gross - pos.contracts * cfg.commission_per_side
                trades.append(
                    dict(
                        ...
                        GrossPnL=gross,
                        NetPnL=gross - pos.contracts * cfg.commission_per_side,
                        Note="roll_force_flat",
                    )
                )
                note = "roll_force_flat_exit_long"
                pos = None
            else:
                slip = slip_points(pos.contracts, stressed, cfg.exit_slip_multiplier)
                exit_price = o + slip
                gross = points_to_dollars(
                    pos.entry_price - exit_price, pos.contracts, cfg.multiplier
                )
                cash += gross - pos.contracts * cfg.commission_per_side
                trades.append(
                    dict(
                ...
                note = "roll_force_flat_exit_short"
                pos = None

            # Recompute MTM after roll exit
            unrl = unrealized_at(c)
            equity = cash + unrl

        # 1) Exit (stops frozen for day)
        if pos is not None:
            if pos.side == +1:
                ...
                if l <= stop_level:
                    ...
                    note = f"exit_long @ {exit_price:.2f}"
                    pos = None
            else:
                ...
                if h >= stop_level:
                    ...
                    note = f"exit_short @ {exit_price:.2f}"
                    pos = None

        # Recompute MTM after exit
        unrl = unrealized_at(c)
        equity = cash + unrl

        # 2) Entry (gap-aware stop entry fill)
        if pos is None:
            long_trigger = cfg.enable_long and (h >= yh)
            short_trigger = cfg.enable_short and (l <= yl)

            # ---- Eligible trigger diagnostics ----
            if not roll_block:
            ...
            if long_trigger and short_trigger:
                ...
                if roll_block:
                    ...
                    contracts, forced_note = decide_contracts(
                        equity, entry_raw, hard_stop, side_tag="long"
                    )
                    if contracts <= 0:
                        note = "no_size_long"
                    else:
                        ...
                        cash -= contracts * cfg.commission_per_side
                        ...
                        if l <= stop_level:
                            ...
                            gross = points_to_dollars(
                                exit_price - entry, contracts, cfg.multiplier
                            )
                            cash += gross - contracts * cfg.commission_per_side
                            trades.append(
                                dict(
                                    EntryDate=date,
                                    ExitDate=date,
                                    ...
                                    Contracts=contracts,
                                    GrossPnL=gross,
                                    NetPnL=gross - contracts * cfg.commission_per_side,
                            ...
                            pos = Position(
                                +1, entry, contracts, hard_stop, trail_stop, date
                            )
                            note = forced_note or f"enter_long @ {entry:.2f}"
                ...
                if roll_block:
                    ...
                    contracts, forced_note = decide_contracts(
                        equity, entry_raw, hard_stop, side_tag="short"
                    )
                    if contracts <= 0:
                        note = "no_size_short"
                    else:
                        ...
                        if h >= stop_level:
                            ...
                            exit_price = stop_level + slip_exit
                            gross = points_to_dollars(
                                entry - exit_price, contracts, cfg.multiplier
                            )
                            cash += gross - contracts * cfg.commission_per_side
                            trades.append(
                                dict(
                            ...
                            note = forced_note or f"enter_short @ {entry:.2f}"

        # 3) End-of-day trail update (applies next day)
        if pos is not None:
            if pos.side == +1:
...
def main():
    cfg = Config(
        ...
        min_contracts_on_signal=0,  # debug-only off
        max_leverage=2.0,
        margin_rate=0.07,
        margin_buffer=0.80,
        ...
        entry_slip_multiplier_on_breakout_day=2.0,  # locked
        stress_mode=True,
        stress_tr_atr_threshold=1.8,
        stress_slip_multiplier=1.5,
        roll_force_flat=False,  # locked
        roll_avoid_entries=True,  # locked
        roll_bdays_before=5,
        roll_window_bdays=2,
    )

    if not cfg.symbols:
        raise ValueError("Config.symbols must be non-empty.")

    print("========== Daily Breakout Playground (v7.6) ==========")
    ...
    print(
        f"Costs: base_slip={cfg.base_slippage_points}, slip_k_sqrt={cfg.slip_k_sqrt}, commission/side={cfg.commission_per_side}"
    )
    print(
    ...
    for sym in cfg.symbols:
        ...
        print("\nTrigger diagnostics (RAW, not entry-eligible):")
        ...
        print("\nTrigger diagnostics (ELIGIBLE = ready & flat & not roll-blocked):")
        ...
        print("\nSizing distribution (on entry-attempt days):")
        ...
        try:
            ...
            res["Equity"].plot(title=f"{sym} Equity (MTM) - v7.6")
            plt.show()
