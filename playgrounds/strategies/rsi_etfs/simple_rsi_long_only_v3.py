...
Implements changes A–D + Volatility Gating (E):

A) Baseline + exposure stats
   - Computes Buy & Hold (per ticker) equity curve (using Adj Close).
   - Prints: % time in market, average exposure, median exposure, etc.
...
D) Entry threshold + optional scaling-in
   - entry_rsi threshold configurable.
   - Optional tiered sizing by "how oversold" the signal RSI is.

E) Volatility gating (crisis filter)
   - Entry signal must ALSO satisfy a volatility regime gate computed on the same ticker:
       - Default: realized vol(20d) > realized vol(252d)
   - This is intended to filter out "normal pullbacks" and keep the overlay for stress regimes.

Notes / limitations:
- Daily OHLC only. TP/SL uses High/Low; when both hit same day, we assume stop-first by default.
  Fills at exact levels (still optimistic vs gaps).
...
class Config:
    ...
    vol_return_col: str = "Close"  # use "Close" (default) or "Adj Close"
    vol_short_window: int = 20
    vol_long_window: int = 252
    vol_gate_ratio_min: float = 1.0  # gate is vol_short/vol_long > this threshold
    vol_gate_strict: bool = (
        True  # if False, allow entry when vol_long is NaN (early samples)
    )
...
def realized_vol_from_price(price: pd.Series, window: int) -> pd.Series:
    """
    Annualized realized volatility from simple daily returns.
    Using simple returns is fine for gating; log returns would be similar.
    """
    r = price.pct_change()
    return r.rolling(window).std(ddof=0) * math.sqrt(252.0)
...
def _vol_gate_ok(cfg: Config, row: pd.Series) -> bool:
    """
    E) Return True if volatility gate allows entry on this signal date.
    We require vol_short / vol_long > threshold.

    If strict:
      - require both vols finite
    else:
      - allow if long vol is NaN (early history), but still require short vol finite
    """
    if not cfg.use_vol_gate:
        return True

    vs = row.get("VOL_S", np.nan)
    vl = row.get("VOL_L", np.nan)

    if cfg.vol_gate_strict:
        if not (pd.notna(vs) and pd.notna(vl) and vs > 0 and vl > 0):
            return False
    else:
        # allow entries even if long window not available yet
        if not (pd.notna(vs) and vs > 0):
            return False
        if not (pd.notna(vl) and vl > 0):
            # long vol missing -> treat as pass
            return True

    ratio = float(vs) / float(vl)
    return ratio > cfg.vol_gate_ratio_min
...
def backtest(
    ...
    for t, df in raw.items():
        ...
        # RSI on Close (signal uses Close)
        d["RSI"] = rsi_wilder(d["Close"], cfg.rsi_period)

        # E) realized vol gate on chosen price column
        price_col = cfg.vol_return_col
        if price_col not in d.columns:
            price_col = "Close"
        price = d[price_col].astype(float)

        d["VOL_S"] = realized_vol_from_price(price, cfg.vol_short_window)
        d["VOL_L"] = realized_vol_from_price(price, cfg.vol_long_window)
    ...
    for i, dt in enumerate(common_index):
        ...
        for t in cfg.tickers:
            ...
            if pd.notna(rsi_val) and float(rsi_val) < cfg.entry_rsi:
                # E) Vol gate must also pass (on signal date)
                if not _vol_gate_ok(cfg, row):
                    continue
...
def print_summary(
    ...
    # Buy & Hold metrics
    bh_metrics = {
        col: compute_metrics_from_equity(buyhold_df[col]) for col in buyhold_df.columns
    }
    ...
    if cfg.use_vol_gate:
        print(
            "Vol gate                : ON "
            f"(vol{cfg.vol_short_window} / vol{cfg.vol_long_window} > {cfg.vol_gate_ratio_min}, "
            f"price={cfg.vol_return_col}, strict={cfg.vol_gate_strict})"
        )
    else:
        print("Vol gate                : OFF")
...
def main():
    ...
    cfg.default_position_fraction = 0.0

    # E) vol gate (recommended ON for crisis overlay behavior)
    cfg.use_vol_gate = True
    cfg.vol_return_col = "Close"
    cfg.vol_short_window = 20
    cfg.vol_long_window = 252
    cfg.vol_gate_ratio_min = 1.0
    cfg.vol_gate_strict = True
