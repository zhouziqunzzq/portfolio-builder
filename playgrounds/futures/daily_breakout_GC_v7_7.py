"""
Daily Breakout Futures Playground (v7.7.1) — GC=F via yfinance (daily bars)

Fixes vs v7.7:
1) CRITICAL: "Both-trigger skip" is now applied BEFORE trend gating.
   - If today's range hits BOTH yesterday high and yesterday low, we ALWAYS skip entries.
   - This restores the intended "outside day / chaos day" avoidance that trend gating accidentally removed.

2) Diagnostics: sizing distribution now reflects the CHOSEN contract type (GC vs MGC),
   not just the initial GC attempt (which can be 0 and was biasing stats).
...
Default params: aligned with your v7.6 mainline (risk=3%, entry_breakout_mult=2.0, stress TR/ATR>=1.8 => x1.5,
roll_force_flat=False, roll_avoid_entries=True).
"""
...
def backtest_one(
    ...
    def risk_desired_contracts(
        equity: float, entry_price: float, hard_stop: float, multiplier: float
    ) -> int:
    ...
    def decide_contracts_and_kind(
        ...
        desired_gc = risk_desired_contracts(
            equity, entry_price, hard_stop, cfg.gc_multiplier
        )
        if desired_gc <= 0:
        ...
        desired_mgc = risk_desired_contracts(
            equity, entry_price, hard_stop, cfg.mgc_multiplier
        )
        capped_mgc = (
            cap_contracts_by_constraints(
    ...
    for date, row in df.iterrows():
        ...
        trend_ready = (not cfg.trend_filter_enabled) or pd.notna(sma1)
        ...
        sma1 = float(sma1) if cfg.trend_filter_enabled else float("nan")
        ...
        # ----------------------------
        # Entry logic (v7.7.1 fixes)
        # ----------------------------
        if pos is None:
            # ELIGIBLE diagnostics require: ready & flat & not roll-blocked & passes trend filter.
            # BUT: both-trigger skip is independent of trend filter, and happens first.
            ...
            if long_raw and short_raw:
                ...
                # 2) Apply trend gating now
                long_trigger = long_raw and trend_long_ok
                short_trigger = short_raw and trend_short_ok
                ...
                if not roll_block:
                    ...
                    if long_trigger and short_trigger:
                        # With both-trigger skip already handled, this should be rare/never.
                ...
                if long_trigger:
                    if roll_block:
                        ...
                        (
                            ...
                            desired_chosen,
                            capped_chosen,
                        ) = decide_contracts_and_kind(equity, entry_raw, hard_stop)
                    ...
                    # No eligible trigger (could be due to trend gate)
                    pass
...
def main():
    ...
    print("========== Daily Breakout Playground (v7.7.1) ==========")
    ...
    print(
        f"Trend filter: enabled={cfg.trend_filter_enabled}, SMA_window={cfg.trend_sma_window}"
    )
    print()
    ...
    for sym in cfg.symbols:
        ...
        try:
            ...
            res["Equity"].plot(title=f"{sym} Equity (MTM) - v7.7.1")
            plt.show()
